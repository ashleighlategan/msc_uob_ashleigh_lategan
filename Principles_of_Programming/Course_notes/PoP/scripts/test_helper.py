...
class TestCaseSpec:
    ...
    def add_tests(self, tests, secret_tests, func_name, local_vars):
        func = local_vars[func_name]  # no eval with empty globals
...
class TestSpec:
    ...
    def generate_tests(self):
        ...
        local_vars = {}
        exec(self.__code, local_vars, local_vars)
        assert self.__name in local_vars
        #solution_function = eval(self.__name, {}, local_vars)

        for input_spec in self.__inspecs:
            input_spec.add_tests(tests, secret_tests, self.__name, local_vars)
